...
| Status and timing | Shows `○⠋` working, `🟠` permission, `🟢` ready until the next reply, `🔴` interrupted, idle age colors `🟨` → `🟧` → `🟫`, and the active or last turn time in the status bar |
...
In Codex and regular shells, click and drag selects text, copies it to the macOS clipboard on release, and immediately returns to live output. Claude keeps its own mouse selection so its alternate-screen UI never opens an empty tmux copy view. Empty tmux selections leave the existing clipboard untouched. Double click copies one word. Keyboard selection uses `v`, arrow keys, and `y` to copy. While scrollback or keyboard copy mode is active, the status bar shows **Jump to bottom**. Click it, or press `q` or `Escape`, to return to live output.
